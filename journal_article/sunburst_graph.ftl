...
$(document).ready(function(){
  ...
  // Circle to help with the mouseleave
  svg.append("svg:circle")
        .attr("r", radius)
        .style("opacity", 0);

  // Defines the field from JSON with the values
  var partition = d3.layout.partition().value(function(d) { return d.size; });

  // Functions to draw the circles
  var arc = d3.svg.arc()
  ...
  d3.json(dataURL, function(error, root) {
    var g = svg.selectAll("g").data(partition.nodes(root).filter(
      function(d) {
  ...
  function computeTextRotation(d) {
    rotation =  (x(d.x + d.dx / 2) - Math.PI / 2) / Math.PI * 180;
    console.log(this);
    if (rotation > 90)
      return rotation - 180;
